...
const BACKEND_NAME: &str = "reaper-mcp-backend.exe";
const BACKEND_PORT: u16 = 10797;
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("reaper-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/reaper-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'reaper-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'reaper-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM reaper-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM reaper-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name reaper-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM reaper-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    let workdir = install_dir_from_backend(&backend_path);
