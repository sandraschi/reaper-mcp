import { Circle, Headphones, List, VolumeX } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground italic text-sm">Total Count: {tracks.length}</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {tracks.map((track) => (
            <div
              key={track.id}
              className="flex items-center justify-between p-4 border rounded-lg bg-slate-500/5 group hover:bg-slate-500/10 transition-colors"
            >
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="ghost"
                  className={cn("h-8 w-8 p-0", track.muted && "text-yellow-500 bg-yellow-500/10")}
                >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className={cn("h-8 w-8 p-0", track.solo && "text-emerald-500 bg-emerald-500/10")}
                >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className={cn("h-8 w-8 p-0", track.armed && "text-red-500 bg-red-500/10")}
                >
