...
const WELCOME_MSG =
  "Hi! I'm your Reaper MCP assistant. Ask me to automate audio production, manage tracks, apply effects, or control your DAW workflow.";
...
const PERSONALITIES = [
  {
    ...
    prompt:
      "You are an experienced audio engineer for Reaper. Help users with recording, mixing, routing, signal processing, and achieving professional sound. Be technical and precise.",
  },
  {
    id: "midi-specialist",
    label: "MIDI Specialist",
    prompt:
      "You are a MIDI and composition specialist for Reaper. Help users create, edit, and sequence MIDI, work with virtual instruments, and compose music effectively.",
  },
  {
    id: "quick-summarizer",
    label: "Quick Summarizer",
    prompt:
      "You specialize in distilling complex Reaper topics into 3-bullet summaries: (1) what the feature does, (2) how to use it, (3) pro tip.",
  },
...
const EXAMPLE_PROMPTS = [
  {
    ...
    prompts: [
      ...
      "Route track 3 to a group bus with reverb send",
      "Duplicate the selected track with all FX and routing",
    ],
  },
  {
    group: "Effects",
    prompts: [
      "Apply reverb to the vocal bus with 2.5s decay",
      "Add a sidechain compressor to the bass track",
      "Show me the FX chain on the master track",
    ],
  },
  {
    group: "Mix",
    prompts: [
      "Set the master volume to -6dB and export the mix",
      "Create a stem render of drums, bass, and vocals",
      "Automate the track volume to fade out over 8 bars",
    ],
  },
];
...
export function Chat() {
  ...
  const systemPrompt =
    personalityId === "custom"
      ? customPrompt ||
        "You are a helpful assistant for Reaper MCP, a DAW automation server for Reaper audio production."
      : personality.prompt;
  ...
  const exportChat = useCallback(() => {
    ...
    a.href = url;
    a.download = `reaper-mcp-chat-${Date.now()}.txt`;
    a.click();
        ...
        <div className="flex items-center gap-2">
          {ollamaUp === null ? (
            ...
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-green-500" /> Ollama :11434
            </span>
          ) : (
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-red-500" /> Ollama not detected
            </span>
          )}
          <input
            value={model}
