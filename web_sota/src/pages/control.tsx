import { Activity, Battery, Play, Power, RefreshCw, Square, Video, Wifi, Zap } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="teleop" className="space-y-4">
        <TabsList className="bg-slate-900/50 border border-slate-800">
          <TabsTrigger value="teleop" className="data-[state=active]:bg-slate-800">
            Teleoperation
          </TabsTrigger>
          ...
          <Card className="border-slate-800 bg-slate-950/50">
            ...
            <CardContent>
              <div className="font-mono text-sm text-slate-400 space-y-2">
                <p>
                  [10:42:15] <span className="text-emerald-500">INFO</span> Unitree H1 connected to control server
                </p>
                <p>
                  [10:41:22] <span className="text-yellow-500">WARN</span> Go2 Alpha latency spike (145ms)
                </p>
                <p>
                  [10:40:05] <span className="text-blue-500">DEBUG</span> Mqtt connection established on
                  wss://robotics.local
                </p>
              </div>
            </CardContent>
  ...
  return (
    <Card className="border-slate-800 bg-slate-950/50 backdrop-blur-sm transition-all hover:bg-slate-900/50">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-medium text-slate-200">{robot.name}</CardTitle>
        <Badge
          variant="outline"
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Progress
              ...
              indicatorClassName={robot.battery < 20 ? "bg-red-500" : "bg-emerald-500"}
            />
          </div>
        </div>
      </CardContent>
      ...
      <Card className="col-span-3 border-slate-800 bg-slate-950/50">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <Button
              variant="outline"
              className="h-24 w-full flex-col gap-2 border-slate-800 bg-slate-900/50 hover:bg-slate-800 hover:text-white"
            >
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Button size="sm" variant="secondary" className="bg-slate-800 text-slate-300">
                Walk
              </Button>
